...
pub(in crate::now_playing) fn create(now_playing: Rc<RefCell<NowPlaying>>) -> gtk::Box {
    ...
    let controls = gtk::Box::builder().margin_start(8).margin_end(8).build();
    body.append(&controls);
    let skip_backward = Button::builder().hexpand(true).tooltip_text("Previous")
        .child(&Image::builder().icon_name("media-skip-backward").pixel_size(80).build()).build().flat();
    ...
    let skip_forward = Button::builder().hexpand(true).tooltip_text("Next")
        .child(&Image::builder().icon_name("media-skip-forward").pixel_size(80).build()).build().flat();
