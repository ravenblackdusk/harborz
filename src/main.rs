...
use gtk::{Button, MenuButton, Popover, ScrolledWindow, Widget};
use gtk::Align::Fill;
...
fn main() -> Result<ExitCode> {
    ...
    application.connect_activate(|application| {
        ...
        let history: Rc<RefCell<Vec<(GString, GString, Box<dyn AsRef<Widget>>)>>> = Rc::new(RefCell::new(Vec::new()));
        back_button.connect_clicked({
            let history = history.clone();
            let scrolled_window = scrolled_window.clone();
            let window_title = window_title.clone();
            move |_| {
                if let Some((title, subtitle, last_child)) = history.borrow_mut().pop() {
        ...
        collection_button.connect_clicked({
            ...
            move |_| {
                ...
                window_title.set_subtitle("Collection");
                menu_button.popdown();
            }
        });
        home::set_body(&window_title, &scrolled_window, history, &media_controls);
        main_box.append(&bar);
