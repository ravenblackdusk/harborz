...
use adw::{MessageDialog, ResponseAppearance::Destructive};
use adw::gio::{SimpleAction, SimpleActionGroup};
use adw::glib::{ControlFlow::*, timeout_add_local};
use adw::prelude::*;
use gtk::{Button, CheckButton, GestureClick, GestureZoom, Overlay, ProgressBar};
...
impl MergeState {
    ...
    pub(in crate::body) fn new<G: Fn(Vec<Option<&String>>, bool) -> Vec<(Song, Collection)> + Send + Clone + 'static,
        M: Fn(Song, &str) + Send + Clone + 'static>(merge_entity: Entity, state: Rc<State>, title: Arc<String>,
            ...
            move |_| {
                ...
                action.connect_activate({
                    ...
                    move |action, state| {
                        ...
                        dialog.set_response_enabled(MERGE, true);
                    }
                });
                ...
                let first_check_button = Self::check_button(&entities[0], &gtk_box);
                for entity in &entities[1..] {
                    let check_button = Self::check_button(entity, &gtk_box);
                    check_button.set_group(Some(&first_check_button));
                }
                dialog.add_response("cancel", "_Cancel");
                const MERGE: &'static str = "merge";
                dialog.add_response(MERGE, "_Merge");
                dialog.set_response_appearance(MERGE, Destructive);
                dialog.set_response_enabled(MERGE, false);
                dialog.connect_response(Some(MERGE), {
                            ...
                            move || {
                                ...
                                for (i, (song, collection)) in song_collections.into_iter().enumerate() {
                                    ...
                                    if let Some(mut tag) = tag {
                                        match entity_type {
                                            EntityType::Artist => { tag.set_artist(&entity); }
                                            EntityType::Album => { tag.set_album(&entity); }
                                        }
