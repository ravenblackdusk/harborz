...
pub fn frame() -> Frame {
    let collection_grid: Grid = CollectionGrid::new();
...
fn open_add_directories_to_collection_dialog(collection_grid: &Grid) {
...
fn add_directories_to_collection(dialog: &FileChooserDialog, collection_grid: &Grid) -> Result<()> {
